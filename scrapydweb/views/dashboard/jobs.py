...
class JobsView(BaseView):
    ...
    def __init__(self):
        ...
        self.style = (
            style if style in ["database", "classic"] else self.metadata["style"]
        )
        ...
        self.per_page = request.args.get(
            "per_page", default=self.metadata["per_page"], type=int
        )
    ...
    def dispatch_request(self, **kwargs):
        status_code, self.text = self.make_request(
            self.url, auth=self.AUTH, as_json=False
        )
        ...
        self.jobs = [
            dict(zip(JOB_KEYS, job)) for job in re.findall(JOB_PATTERN, self.text)
        ]
    ...
    def set_flash(self):
        ...
        if self.IS_LOCAL_SCRAPYD_SERVER:
            if not self.LOCAL_SCRAPYD_LOGS_DIR:
                flash(
                    (
                        "Set up the LOCAL_SCRAPYD_LOGS_DIR option to speed up the loading of scrapy logfiles "
                        "for the LOCAL_SCRAPYD_SERVER %s" % self.SCRAPYD_SERVER
                    ),
                    self.WARN,
                )
        ...
        if not self.ENABLE_MONITOR and self.SCRAPYD_SERVERS_AMOUNT == 1:
            flash(
                "Set 'ENABLE_MONITOR = True' to enable the monitor feature", self.INFO
            )
    ...
    def get_liststats_datas(self):
        ...
        if js["status"] == self.OK:
            ...
            self.logger.debug(
                "Got datas with %s entries from liststats: %s",
                len(self.liststats_datas),
                js,
            )
        else:
            self.logger.warning(
                "Fail to get datas from liststats: (%s) %s %s",
                js["status_code"],
                js["status"],
                js.get("tip", ""),
            )
    ...
    def create_table(self):
        ...
        if self.Job is not None:
            ...
            self.Job = create_jobs_table(
                re.sub(STRICT_NAME_PATTERN, "_", self.SCRAPYD_SERVER)
            )
    ...
    def handle_jobs_with_db(self):
        try:
            ...
            self.logger.error(
                "Fail to persist jobs in database: %s", traceback.format_exc()
            )
            db.session.rollback()
            ...
            if "is already defined for this MetaData instance" in str(err):
                flash(
                    "Please restart ScrapydWeb to work around this occasional bug!",
                    self.WARN,
                )
    ...
    def handle_unique_constraint(self):
        ...
        for job in self.jobs:  # (Pending, Running) ASC
            ...
            if unique_key in seen_jobs:  # ignore previous
                ...
                unique_key_string = "/".join(
                    list(unique_key) + [start, finish, str(self.node)]
                )
                if start:
                    msg = "Ignore seen running job: %s, started at %s" % (
                        "/".join(unique_key),
                        start,
                    )
                else:
                ...
                if (
                    unique_key_string not in self.metadata["unique_key_strings"]
                ):  # flash only once
        ...
        for job in reversed(self.jobs):  # Finished DESC
            ...
            if unique_key in seen_jobs:  # ignore current
                unique_key_string = "/".join(
                    list(unique_key) + [job["start"], job["finish"], str(self.node)]
                )
                msg = "Ignore seen finished job: %s, started at %s" % (
                    "/".join(unique_key),
                    job["start"],
                )
                self.logger.debug(msg)
    ...
    def db_insert_jobs(self):
        ...
        for job in self.jobs:  # set(self.jobs): unhashable type: 'dict'
            record = self.Job.query.filter_by(
            ...
            if record:
                ...
                if record.deleted == DELETED:
                    if (
                        ...
                        and str(record.start) == job["start"]
                    ):
            ...
            for k, v in job.items():
                ...
                if k in ["start", "finish"]:
                    ...
                    "href_log",
                    "href_items",
                ]:  # <a href='/logs/demo/test/xxx.log'>Log</a>
            ...
            if not job["start"]:
                ...
                try:
                    data = self.liststats_datas[job["project"]][job["spider"]][
                        job["job"]
                    ]
    ...
    def db_clean_pending_jobs(self):
        current_pending_jobs = [
            (job["project"], job["spider"], job["job"])
            for job in self.jobs_backup
            if not job["start"]
        ]
    ...
    def query_jobs(self):
        current_running_job_pids = [
            int(job["pid"]) for job in self.jobs_backup if job["pid"]
        ]
        self.logger.debug("current_running_job_pids: %s", current_running_job_pids)
        self.jobs_pagination = (
            self.Job.query.filter_by(deleted=NOT_DELETED)
            .order_by(
                self.Job.status.asc(),
                self.Job.finish.desc(),
                self.Job.start.asc(),
                self.Job.id.asc(),
            )
        ...
        with db.session.no_autoflush:
            for index, job in enumerate(
                ...
                job.to_be_killed = (
                    True
                    if job.pid and job.pid not in current_running_job_pids
                    else False
                )
                if job.finish:
                    job.url_multinode = url_for(
                        "servers",
                        node=self.node,
                        opt="schedule",
                        project=job.project,
                        version_job=self.DEFAULT_LATEST_VERSION,
                        spider=job.spider,
                    )
                    job.url_action = url_for(
                        "schedule",
                        node=self.node,
                        project=job.project,
                        version=self.DEFAULT_LATEST_VERSION,
                        spider=job.spider,
                    )
                else:
                    job.url_multinode = url_for(
                ...
                job.url_clusterreports = url_for(
                    "clusterreports",
                    node=self.node,
                    project=job.project,
                    spider=job.spider,
                ...
                if job.href_items:
                    ...
                    job.url_items = ""
                job.url_delete = url_for(
                    "jobs.xhr", node=self.node, action="delete", id=job.id
                )
    ...
    def set_jobs_dict(self):
        for (
            ...
            self.jobs_pagination.items
        ):  # Pagination obj in handle_jobs_with_db() > query_jobs()
    ...
    def handle_jobs_without_db(self):
        for job in self.jobs:
            ...
            if not job["finish"]:
                job["url_multinode_stop"] = url_for(
                    "servers",
                    node=self.node,
                    opt="stop",
                    project=job["project"],
