...
def handle_db(app):
    ...
    ] = False  # https://stackoverflow.com/a/33790196/10517783
    app.config[
        "SQLALCHEMY_ECHO"
    ] = True  # http://flask-sqlalchemy.pocoo.org/2.3/config/
    ...
    with db.app.app_context():
        if not Metadata.query.filter_by(version=__version__).first():
            ...
            db.session.commit()
    if (
        time.time() - handle_metadata().get("last_check_update_timestamp", time.time())
        > 3600 * 24 * 30
    ):
...
def handle_route(app):
    def register_view(
        view, endpoint, url_defaults_list, with_node=True, trailing_slash=True
    ):
    ...
    register_view(
        ...
        [
            (
                "servers/getreports/<project>/<spider>/<version_job>",
                dict(opt="getreports"),
            ),
    ...
    register_view(
        DeployXhrView,
        "deploy.xhr",
        [("deploy/xhr/<eggname>/<project>/<version>", None)],
    )
    ...
    register_view(
        ...
        [
            ...
            (
                "projects",
                dict(opt="listprojects", project=None, version_spider_job=None),
            ),
        ],
    )
    ...
    register_view(
        UploadedLogView, "parse.uploaded", [("parse/uploaded/<filename>", None)]
    )
...
def handle_template_context(app):
    ...
    def inject_variable():
        return dict(
            ...
            static_css_dropdown=url_for(
                STATIC, filename="%s/css/dropdown.css" % VERSION
            ),
            static_css_dropdown_mobileui=url_for(
                STATIC, filename="%s/css/dropdown_mobileui.css" % VERSION
            ),
            static_css_icon_upload_icon_right=url_for(
                STATIC, filename="%s/css/icon_upload_icon_right.css" % VERSION
            ),
            static_css_multinode=url_for(
                STATIC, filename="%s/css/multinode.css" % VERSION
            ),
            static_css_stacktable=url_for(
                STATIC, filename="%s/css/stacktable.css" % VERSION
            ),
            ...
            static_css_style_mobileui=url_for(
                STATIC, filename="%s/css/style_mobileui.css" % VERSION
            ),
            static_css_utf8=url_for(STATIC, filename="%s/css/utf8.css" % VERSION),
            static_css_utf8_mobileui=url_for(
                STATIC, filename="%s/css/utf8_mobileui.css" % VERSION
            ),
            static_css_element_ui_index=url_for(
                STATIC,
                filename="%s/element-ui@2.4.6/lib/theme-chalk/index.css" % VERSION,
            ),
            static_js_element_ui_index=url_for(
                STATIC, filename="%s/element-ui@2.4.6/lib/index.js" % VERSION
            ),
            static_js_common=url_for(STATIC, filename="%s/js/common.js" % VERSION),
            static_js_echarts_min=url_for(
                STATIC, filename="%s/js/echarts.min.js" % VERSION
            ),
            static_js_icons_menu=url_for(
                STATIC, filename="%s/js/icons_menu.js" % VERSION
            ),
            # static_js_github_buttons_html=url_for(STATIC, filename='%s/js/github_buttons.html' % VERSION),
            static_js_github_buttons=url_for(
                STATIC, filename="%s/js/github_buttons.js" % VERSION
            ),
            static_js_jquery_min=url_for(
                STATIC, filename="%s/js/jquery.min.js" % VERSION
            ),
            static_js_multinode=url_for(
                STATIC, filename="%s/js/multinode.js" % VERSION
            ),
            static_js_stacktable=url_for(
                STATIC, filename="%s/js/stacktable.js" % VERSION
            ),
            ...
            static_icon_apple_touch=url_for(
                STATIC, filename="%s/icon/spiderman.png" % VERSION
            ),
        )
